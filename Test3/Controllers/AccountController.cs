        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    Debug.WriteLine("UserId: " + User.Identity.GetUserId());
                    Debug.WriteLine("User: " + user);
                    // 根据用户角色和ID构造重定向URL
                    string redirectUrl;
                    if (user.Person_role == "Doctor")
                    {
                        redirectUrl = Url.Action("Doctor_Index", "Appointment", new { id = user.Id });
                    }
                    else if (user.Person_role == "Patient")
                    {
                        redirectUrl = Url.Action("Patient_Index", "Appointment", new { id = user.Id });
                    }
                    else
                    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    return RedirectToAction("success_Login", "Home");
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.RequiresVerification:
                    return RedirectToAction("SendCode", new { ReturnUrl = returnUrl, RememberMe = model.RememberMe });
